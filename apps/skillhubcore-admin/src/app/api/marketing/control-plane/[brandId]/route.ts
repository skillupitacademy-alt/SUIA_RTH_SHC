...
export async function GET(
  ...
  try {
    const response = await fetch(`${baseUrl}/public/marketing/control-plane/${brandId}`, {
      headers: {
    ...
    return NextResponse.json(
      {
        ok: false,
        error: error instanceof Error ? error.message : 'marketing_control_plane_proxy_failed',
      },
      { status: 500 },
    );
  }
