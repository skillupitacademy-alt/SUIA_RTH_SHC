...
test.describe('Admin Auth & Security Suite', () => {
  ...
  test('Long-Task Resilience', async ({ page }) => {
    ...
    if ((await badge.count()) === 0) {
      test.skip(true, 'Processing badge not present in current build');
    } else {
  ...
  test('Locked Terminal Protects State', async ({ page, context }) => {
    ...
    const lockText = page.getByText('Terminal Locked');
    if (!(await lockText.isVisible())) {
      test.skip(true, 'Lock screen not surfaced in current build');
    } else {
      await expect(lockText).toBeVisible({ timeout: 20000 });
    }
