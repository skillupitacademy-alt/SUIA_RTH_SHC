        ...
        <div className="space-y-4">
            ...
            <div className="bg-white/50 backdrop-blur-xl border border-primary/10 p-6 rounded-[2rem] shadow-xl relative overflow-hidden flex items-center justify-between">
                <div className="flex items-center gap-4 flex-1">
                    <div className="p-2 rounded-xl bg-indigo-50 text-indigo-500 shadow-sm border border-indigo-100">
                        <BookOpen className="w-5 h-5" />
                    </div>
                    <div className="relative flex-1 max-w-md group">
                        <input
                            type="text"
                            placeholder="SEARCH_SUBJECTS..."
                            value={searchQuery}
                            onChange={(e) => { setSearchQuery(e.target.value); setPage(1); }}
                            className="w-full bg-slate-50 border-none rounded-2xl px-5 py-3 text-[11px] font-black tracking-widest text-[#1A1A1A] placeholder:text-slate-300 focus:ring-2 focus:ring-indigo-500/10 transition-all outline-none border border-transparent shadow-inner"
                        />
                ...
                <table className="w-full text-left">
                    <thead>
                        <tr className="border-b border-primary/5 bg-primary/5">
                            ...
                            <th className="p-6 text-[11px] font-black uppercase tracking-widest text-muted-foreground">Parent Domain</th>
                            <th className="p-6 text-[11px] font-black uppercase tracking-widest text-muted-foreground">Order</th>
                            ...
                            <tr key={item.id} className="group hover:bg-primary/5 transition-colors">
                                ...
                                <td className="p-6">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-sm font-medium text-muted-foreground">{item.domain?.name || 'N/A'}</span>
                                    </div>
                                </td>
                                <td className="p-6 text-sm font-bold text-muted-foreground">#{item.order}</td>
                                <td className="p-6">
