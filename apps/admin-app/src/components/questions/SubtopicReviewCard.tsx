...
import type { Domain, Subject, Subtopic, Topic } from '@/types/domain';

type SubtopicCard = Subtopic & {
    topic?: Topic & {
...
interface SubtopicReviewCardProps {
    ...
    onDeleteRequest: (subtopic: SubtopicCard) => void;
            ...
            <div className="px-8 py-5 border-b border-slate-100 flex flex-wrap items-center justify-between gap-4 bg-slate-50/50">
                <div className="flex items-center gap-5">
                    ...
                    <div className="flex items-center">
                        <input
                            ...
                            aria-label={`Select subtopic ${subtopic.name ?? 'item'}`}
                        />
                    </div>
                    ...
                    <div className="flex flex-col">
                        <div className="flex items-center gap-2 text-[10px] uppercase font-black tracking-widest text-slate-500">
                            ...
                            <span>{(subtopic.topic?.subject?.domain?.name != null && subtopic.topic.subject.domain.name !== '') ? subtopic.topic.subject.domain.name : 'N/A'}</span>
                            <span className="opacity-30">/</span>
                            <BookOpen size={10} className="text-slate-500" />
                            <span>{(subtopic.topic?.subject?.name != null && subtopic.topic.subject.name !== '') ? subtopic.topic.subject.name : 'N/A'}</span>
                            <span className="opacity-30">/</span>
                            <Hash size={10} className="text-teal-500" />
                            <span className="text-teal-600 font-black">{(subtopic.topic?.name != null && subtopic.topic.name !== '') ? subtopic.topic.name : 'N/A'}</span>
                        </div>
                ...
                <div className="flex items-center gap-2">
                    <div className={cn(
                        ...
                        (subtopic.status != null && statusColors[subtopic.status] != null) ? statusColors[subtopic.status] : statusColors.active
                    )}>
            ...
            <div className="p-8 flex gap-8">
                ...
                <div className="w-64 flex-shrink-0 space-y-4">
                    <div className="p-5 rounded-3xl bg-slate-50 border border-slate-200 flex flex-col gap-4">
                        <div className="flex flex-col gap-1">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm font-black text-slate-700">D-Lvl: {(subtopic.depthLevel != null && subtopic.depthLevel !== 0) ? subtopic.depthLevel : 1}</span>
                            </div>
                        </div>
                    ...
                    <div className="flex items-center gap-2 px-4 py-3 rounded-2xl bg-slate-50/50 border border-slate-100 text-[10px] font-bold text-slate-500">
                        <Clock size={12} />
                        <span>Created {subtopic.createdAt != null ? formatTimeAgo(subtopic.createdAt) : 'Unknown'}</span>
                    </div>
                </div>
                ...
                <div className="flex-1 space-y-6">
                        ...
                        <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1">
                                ...
                                <p className="text-sm text-slate-600 font-medium leading-relaxed max-w-2xl">
                                    {(subtopic.description != null && subtopic.description !== '') ? subtopic.description : 'No structural definition provided for this leaf node.'}
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-3 pl-12">
                        <div className="px-4 py-2 rounded-xl bg-slate-50 border border-slate-100 flex items-center gap-2">
                            ...
                            <span className="text-xs font-bold text-slate-700">{(subtopic.questionsCount != null && subtopic.questionsCount !== 0) ? subtopic.questionsCount : 0}</span>
                        </div>
                    </div>
                </div>
            </div>
