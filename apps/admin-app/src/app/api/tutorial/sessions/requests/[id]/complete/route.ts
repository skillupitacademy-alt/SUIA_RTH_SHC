...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const result = await liveSessionService.completeRequest(session.userId, id);
    return NextResponse.json({ data: result }, { status: 200 });
  } catch (error) {
    if (error instanceof SessionRequestNotFoundError) {
      return NextResponse.json({ error: error.message }, { status: 404 });
    }
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to complete request' }, { status: 500 });
  }
}
