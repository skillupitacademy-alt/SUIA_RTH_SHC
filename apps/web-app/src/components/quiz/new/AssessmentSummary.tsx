...
export function AssessmentSummary({
    ...
    return (
        <aside className="sticky top-8 w-full min-h-[750px] flex flex-col">
            <div className="glass-morphism pink-glow rounded-[2.5rem] p-8 space-y-8 relative overflow-hidden group transition-all duration-500 hover:shadow-[0_0_50px_rgba(255,45,85,0.15)] flex-1 flex flex-col">
                {/* Visual Aura */}
                ...
                <div className="relative z-10 space-y-6 flex-1 flex flex-col">
                        ...
                        {selectedSubjects.length > 0 && (
                            <div className="flex flex-col gap-2">
                                <div className="flex items-center gap-3 text-muted-foreground">
                                    ...
                                    <span className="text-xs font-bold font-inter uppercase tracking-widest opacity-60">Subjects</span>
                                </div>
                                <div className="flex flex-wrap gap-2 pl-7">
                                    {selectedSubjects.map(s => (
                                        <span key={s} className="px-2 py-1 bg-gray-50 text-[10px] font-bold text-gray-500 rounded-md border border-gray-100">{s}</span>
                                    ))}
                                </div>
                            </div>
                        )}
                        ...
                        {selectedTopics.length > 0 && (
                            <div className="flex flex-col gap-2">
                                <div className="flex items-center gap-3 text-muted-foreground">
                                    ...
                                    <span className="text-xs font-bold font-inter uppercase tracking-widest opacity-60">Target Topics</span>
                                </div>
                                <div className="flex flex-wrap gap-2 pl-7">
                                    {selectedTopics.map(t => (
                                        <span key={t} className="px-2 py-1 bg-[#FF2D55]/5 text-[10px] font-bold text-[#FF2D55]/60 rounded-md border border-[#FF2D55]/10">{t}</span>
                                    ))}
                                </div>
                            </div>
                        )}
                    ...
                    <button
                        ...
                        className={cn(
                            "w-full py-5 rounded-2xl bg-gradient-to-r from-[#FF2D55] to-[#D4145A] text-white font-bold font-outfit uppercase tracking-wider shadow-[0_10px_30px_rgba(255,45,85,0.3)] flex items-center justify-center gap-3 transition-all active:scale-95 disabled:grayscale disabled:opacity-50 group hover:shadow-[0_15px_40px_rgba(255,45,85,0.45)] mt-auto",
