 ...
 * 
 * SkillUp BFF - Proxies to centralized API server for visit tracking
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      const error = await response.json().catch(() => ({ error: 'Failed to track visit' }));
      return NextResponse.json(error, { status: response.status });
    }
    
    return NextResponse.json(await response.json());
  } catch (error) {
    console.error('[SkillUp ILS] trackVisit error:', error);
