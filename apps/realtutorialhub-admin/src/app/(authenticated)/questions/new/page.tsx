...
export default function QuestionEntryPage() {
    ...
    const handleCreateQuestion = async (formData: QuestionFormData) => {
        ...
        try {
            ...
            if (mainContent != null) { mainContent.scrollTo({ top: 0, behavior: 'smooth' }); }
            else { window.scrollTo({ top: 0, behavior: 'smooth' }); }

            setTimeout(() => {
                setStatus(prev => (prev != null && prev.type === 'success') ? null : prev);
            }, 6000);
    ...
    const isUnlocked = selection.subtopicId != null && selection.subtopicId !== '';

    return (
        <div className="w-full relative">

            {/* Status Banner */}
            {status != null ? <div className={`
