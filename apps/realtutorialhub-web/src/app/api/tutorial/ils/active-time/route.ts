 ...
 * 
 * RTH BFF - Proxies to centralized API server for active time recording
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const url = `${apiUrl}/tutorial/ils/active-time`;
    ...
    if (!response.ok) {
      const error = await response.json().catch(() => ({ error: 'Failed to record active time' }));
      return NextResponse.json(
        { error: error.error || 'Active time recording failed' },
    ...
    console.error('[RTH ILS] recordActiveTime error:', error);
    return NextResponse.json(
