...
describe('LearnerProgressPanel', () => {
  ...
  it('calls the start tier api when the start button is clicked', async () => {
    const fetchMock = vi.fn()
      ...
      .mockResolvedValueOnce(jsonResponse({ data: { id: 'progress-1', status: 'in_progress' } }))
      .mockResolvedValueOnce(
        jsonResponse({
          data: {
            locked: false,
            ...
            tierStatus: {
              simple: { status: 'in_progress', isUnlocked: true, startedAt: null, completedAt: null },
              mixed: { status: 'not_started', isUnlocked: false, startedAt: null, completedAt: null },
              intermediate: { status: 'not_started', isUnlocked: false, startedAt: null, completedAt: null },
              expert: { status: 'not_started', isUnlocked: false, startedAt: null, completedAt: null },
            },
    ...
    expect(startCall[0]).toBe(`/api/tutorial/assignments/${subtopicId}/start`);
    expect(JSON.parse(startCall[1]?.body as string)).toEqual({ difficulty: 'simple' });
  ...
  it('calls the complete tier api and shows the next unlocked tier', async () => {
    const fetchMock = vi.fn()
      ...
      .mockResolvedValueOnce(jsonResponse({ data: { progress: { id: 'progress-1', status: 'self_completed' }, nextUnlockedTier: 'mixed' } }))
      .mockResolvedValueOnce(
        jsonResponse({
          data: {
            locked: false,
            assignments: [{ id: 'assignment-1', question: 'What is a promise?' }],
            progress: { status: 'self_completed' },
            tierStatus: {
              simple: { status: 'self_completed', isUnlocked: true, startedAt: null, completedAt: null },
              mixed: { status: 'not_started', isUnlocked: true, startedAt: null, completedAt: null },
              intermediate: { status: 'not_started', isUnlocked: false, startedAt: null, completedAt: null },
              expert: { status: 'not_started', isUnlocked: false, startedAt: null, completedAt: null },
            },
          },
    ...
    expect(completeCall[0]).toBe(`/api/tutorial/assignments/${subtopicId}/complete`);
    expect(JSON.parse(completeCall[1]?.body as string)).toEqual({ difficulty: 'simple' });
  ...
  it('submits help requests for the current assignment', async () => {
    const fetchMock = vi.fn()
      ...
      .mockResolvedValueOnce(jsonResponse({ data: { id: 'help-1' } }))
      .mockResolvedValueOnce(
        jsonResponse({
          data: {
            locked: false,
    ...
    await waitFor(() => expect(fetchMock).toHaveBeenCalledTimes(3));
    const helpCall = fetchMock.mock.calls[1];
    expect(helpCall[0]).toBe('/api/tutorial/assignments/help');
    expect(JSON.parse(helpCall[1]?.body as string)).toEqual({
      subtopicId,
      assignmentId: 'assignment-1',
      question: 'Need help understanding promise chaining.',
    });
