 ...
 * 
 * Record active time spent on a navigation node.
 * 
...
import { recordActiveTimeBodySchema } from '@/schemas/ils.schemas';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const parsed = recordActiveTimeBodySchema.safeParse(body);
    if (!parsed.success) {
      return NextResponse.json(
    ...
    const progress = await service.recordActiveTime(
      identity,
      parsed.data.navigationNodeId,
      parsed.data.subtopicId,
      parsed.data.incrementSeconds
    );
    ...
    console.error('[ILS API] recordActiveTime error:', error);
    return NextResponse.json(
