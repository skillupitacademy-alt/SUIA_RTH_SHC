...
export default function ResetPasswordPage() {
  ...
  const token = searchParams.get('token') ?? searchParams.get('_token') ?? '';
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch('/api/auth/reset-password', {
        ...
        body: JSON.stringify({ token, password, platform: 'realtutorialhub' }),
      });
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,rgba(255,75,145,0.16),transparent_30%),linear-gradient(180deg,#fff7fb_0%,#ffffff_58%)] px-6 py-16">
      <div className="mx-auto max-w-2xl rounded-[32px] border border-rose-100 bg-white p-10 shadow-[0_30px_80px_rgba(255,75,145,0.12)]">
        <p className="text-sm font-bold uppercase tracking-[0.35em] text-rose-500">RealTutorialHub Recovery</p>
        ...
        {!token ? (
          ...
          <div className="mt-6">
            ...
            <Link href="/login" className="mt-6 inline-block rounded-full bg-rose-500 px-5 py-3 text-sm font-bold text-white transition hover:bg-rose-600">
              Sign in now
            </Link>
          </div>
        ) : (
