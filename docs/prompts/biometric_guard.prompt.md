...
## PHASE 1 CARRY-FORWARD (Security Hardening)

> Complete these baseline items before implementing biometric/advanced auth.

1. **CSRF Cookie Attribute Audit**:
   - Final pass on `SameSite` attributes for cross-domain vs same-site deployment scenarios.
   - Verify `accessToken` and `csrfToken` cookie separation.

2. **Login Attempt Indexing**:
   - Ensure `login_attempts` table has index on `user_id` to support high-speed rate limiting checks.
