...
## PHASE 1 CARRY-FORWARD (Database Resilience)

> These baseline timeouts must be active to protect the primary DB during surges.

1. **Server-Side Statement Timeout**:
   - Add `statement_timeout: 30000` (30s) to the Pool connection parameters in `packages/db/src/index.ts`.

2. **Application-Level Timeout Application**:
   - Locate `ExamEngine.submitAnswer` and `ScoringEngine.calculateResults`.
   - Wrap these write operations with the `withTimeout()` utility for 15s/30s limits respectively.
