...
async function run() {
  ...
  `);
  scriptLogger.info('=== REPORTS TABLE COLUMNS ===');
  scriptLogger.info(cols.rows.map(r => r.column_name).join(', '));

  // 2. ALL reports
  scriptLogger.info('\n=== ALL REPORTS ===');
  const reports = await client.query(`SELECT * FROM reports ORDER BY created_at DESC`);
  scriptLogger.info(`Total: ${reports.rowCount}`);
  reports.rows.forEach(r => {
    scriptLogger.info(`  ${(r.attempt_id || '').slice(0, 8)} | ${(r.status || '').padEnd(12)} | err: ${r.error_stage || 'none'} | file: ${!!r.file_ref} | updated: ${r.updated_at}`);
  });
  ...
  exams.rows.forEach(e => {
    ...
    scriptLogger.info(`  ${e.id.slice(0, 8)} | report: ${rpt}`);
  });
  ...
  if (redisUrl && redisToken) {
    ...
    scriptLogger.info(JSON.stringify(allKeys.result, null, 2));
  }

  await client.end();
}

run().catch(scriptLogger.error);
