...
## PART F: Identified Gaps (Not Covered by Any Existing Phase)

These 8 areas were identified as missing from all existing architecture docs:

| # | Gap | Why It Matters | Priority |
|---|---|---|---|
| G1 | **Accessibility (WCAG 2.1)** | Educational platforms may legally require WCAG AA compliance. Keyboard nav, ARIA labels, color contrast, screen reader support | 🟠 Medium-High |
| G2 | **Internationalization (i18n)** | No i18n infrastructure exists. Needed to serve non-English markets (translation files, locale-aware formatting, RTL support) | 🟡 Medium |
| G3 | **Disaster Recovery & Backups** | Student exam data is irreplaceable. Need automated DB backups, documented recovery procedures, soft deletes | 🔴 High |
| G4 | **Rate Limiting & DDoS Protection** | API is publicly accessible. Without rate limiting, bots can exhaust Vercel function quota or brute-force auth | 🔴 High |
| G5 | **PWA / Mobile Experience** | Many students access exams from phones. Web App Manifest, Service Worker, responsive audit needed | 🟡 Medium |
| G6 | **Admin Audit Trail** | No record of WHO changed WHAT and WHEN in admin operations. Critical for accountability and compliance | 🟠 Medium-High |
| G7 | **SEO & Social Sharing** | Public-facing pages need meta tags, sitemap.xml, Open Graph tags, and structured data (JSON-LD) | 🟢 Low |
| G8 | **Content Versioning (Question Bank)** | When questions are edited, need version history to know which version a student saw. Critical for fairness | 🟠 Medium-High |
