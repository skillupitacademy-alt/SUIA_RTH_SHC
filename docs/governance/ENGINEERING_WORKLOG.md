...
- Phase F2 (routes, batch 1): replaced console logging with Pino in auth (forgot/reset), security report (CSP), and telemetry routes. Telemetry now logs only route/method (no body). CSP logs warn/error with requestId available via wrapper. Lint clean.\n
- Phase F2 (routes, batch 2 start): converted auth/security/telemetry route consoles to Pino (warn/error only). Admin/quiz route console replacements remain for next batch.\n
