...
The [API state route](file:///d:/onlinewebsites/quiz-platform/apps/api-server/src/app/api/quiz/state/route.ts) now enforces UUID validation for the `examId` query parameter, returning a `422 Unprocessable Entity` status for invalid formats, protecting the database from malformed queries.
