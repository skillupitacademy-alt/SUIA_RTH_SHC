...
if ! command -v jq >/dev/null 2>&1; then
  ...
  echo "   Continuing with basic JSON parsing..."
  HAS_JQ=0
else
  HAS_JQ=1
fi
...
# Deployment state directory (outside repository)
STATE_DIR="/opt/platform/state"
HISTORY_DIR="$STATE_DIR/history"
DEPLOYMENT_STATE="$STATE_DIR/deployment.json"
SERVICE_MAP="$SCRIPT_DIR/../config/service-map.json"
...
echo "🚀 Production Deployment System v2.0"
echo "===================================="
echo ""
...
# Get last deployed commit (with jq if available)
LAST_COMMIT=""
LAST_SHORT=""
...
if [ -f "$DEPLOYMENT_STATE" ]; then
  ...
  if [ -n "$LAST_COMMIT" ] && [ "$LAST_COMMIT" != "null" ]; then
    ...
    echo "📋 Last deployment: $LAST_SHORT"
  else
    IS_FIRST_DEPLOYMENT=1
    echo "📋 First deployment: No previous state found"
  fi
else
  IS_FIRST_DEPLOYMENT=1
  echo "📋 First deployment: No deployment state file"
fi
...
if [ $IS_FIRST_DEPLOYMENT -eq 0 ] && [ "$LAST_COMMIT" = "$CURRENT_COMMIT" ]; then
  echo "✅ No changes detected. Current deployment is up to date."
  exit 0
fi
...
if [ $IS_FIRST_DEPLOYMENT -eq 1 ]; then
  ...
  # Check if last commit exists in history
  if ! git cat-file -e "$LAST_COMMIT" 2>/dev/null; then
    echo "${YELLOW}⚠️  Warning: Last deployed commit $LAST_SHORT not found in history${NC}"
    echo "   This might be after a git gc or repository cleanup"
