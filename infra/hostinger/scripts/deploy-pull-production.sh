#!/usr/bin/env sh
# Pull registry-built images on the VPS and deploy without building locally.
...
for service in $SERVICES; do
  ...
  docker tag "$registry_ref" "${image_name}:latest"
  add_image_to_manifest "$IMAGE_MANIFEST" "$service" "$image_name" "$registry_ref"
  log_success "Pulled $registry_ref as ${image_name}:latest"
done
