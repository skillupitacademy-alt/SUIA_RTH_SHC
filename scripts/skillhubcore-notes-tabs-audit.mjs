...
const ARTIFACT_DIR = 'test-results/skillhubcore-notes-tabs-audit';
const TABS = [
...
const EXPECTATIONS = {
  'Universal Architecture': ['Constitutional Section Architecture', 'Concept Card', 'NotesHero', 'Definition Block', 'CoreDefinition'],
  'Section Sequence': ['Universal Section Sequence', 'Concept Card', 'Definition Block', 'component_grid'],
  'Component Details': ['Fixed Component Architecture', 'Renderer Mapping Configuration', 'NotesHero', 'CoreDefinition'],
  'Learning Progression': ['Learning Progression', 'concept_card', 'definition_block'],
  'Prompt Management': ['Prompt Management', 'Prompt', 'notes.concept_card'],
  'Renderer Mapping': ['Editable Renderer Contract', 'Renderer Decision Preview', 'NotesHero', 'Concept Card'],
  'Validation Rules': ['Validation', 'schema', 'notes'],
  'JSON Schema': ['JSON Schema', 'concept_card', 'definition_block', 'NotesHero'],
};
...
async function selectNotes(page) {
  await page.locator('button').filter({ hasText: 'Educational Arch' }).click();
  await page.getByText('Notes Section', { exact: true }).click();
...
async function main() {
  const hardTimeout = setTimeout(() => {
    console.error(JSON.stringify({ ok: false, error: 'Notes tabs audit hard timeout exceeded.' }, null, 2));
    process.exit(124);
  }, 150000);
  ...
  try {
    ...
    console.log(JSON.stringify({ ok, baseUrl: BASE_URL, section: 'notes', selectedComponent: 'concept_card', results }, null, 2));
