...
async function main() {
  ...
  try {
    ...
    auditReport.findings.tutorial_difficulty_enum = {
      database_columns: difficultyColumns.rows.map((col: any) => ({
    ...
    // Classify each usage
    difficultyColumns.rows.forEach((col: any) => {
  ...
  try {
    ...
    auditReport.findings.section_type_enum = {
      database_columns: sectionTypeColumns.rows.map((col: any) => ({
        table: col.table_name,
        column: col.column_name,
        nullable: col.is_nullable === 'YES'
      })),
    ...
    sectionTypeColumns.rows.forEach((col: any) => {
  ...
  try {
    ...
    auditReport.findings.subsection_type_enum = {
      database_columns: subsectionTypeColumns.rows.map((col: any) => ({
        table: col.table_name,
        column: col.column_name
      })),
    ...
    if (subsectionTypeColumns.rows.length > 0) {
      subsectionTypeColumns.rows.forEach((col: any) => {
        console.log(`      ${col.table_name}.${col.column_name}`);
      });
    }
  ...
  try {
    ...
    auditReport.findings.foreign_keys = {
      ...
      dependencies: foreignKeys.rows.map((fk: any) => ({
        table: fk.table_name,
      ...
      tutorial_sections_references: foreignKeys.rows.filter((fk: any) => fk.foreign_table_name === 'tutorial_sections').length
    };
  ...
  try {
    ...
    auditReport.findings.database_objects = {
      views: views.rows.map((v: any) => ({ name: v.viewname, uses_legacy: true })),
      functions: functions.rows.map((f: any) => ({ name: f.proname, uses_legacy: true })),
      triggers: triggers.rows.map((t: any) => ({ name: t.trigger_name, table: t.event_object_table })),
  ...
  try {
    ...
    // A011: API routes
    const { stdout: apiSectionType } = await execAsync('rg "sectionType" --type ts apps/*/src/app/api -l 2>nul || echo ""');
    const { stdout: apiDifficulty } = await execAsync('rg "difficulty" --type ts apps/*/src/app/api -l 2>nul || echo ""');
    ...
    const { stdout: deliveryDifficulty } = await execAsync('rg "difficulty" --type ts apps/*/src/server/tutorial-delivery.ts -l 2>nul || echo ""');
    const { stdout: cacheDifficulty } = await execAsync('rg "tutorial.*difficulty" --type ts packages/ -l 2>nul || echo ""');
    const { stdout: vectorDifficulty } = await execAsync('rg "difficulty" --type ts packages/db-tutorial/src/vector.service.ts -l 2>nul || echo ""');
    ...
    // A013: Test files
    const { stdout: testSectionType } = await execAsync('rg "sectionType" --type ts --glob "**/*.test.ts" -l 2>nul || echo ""');
    const { stdout: testDifficulty } = await execAsync('rg "difficulty" --type ts --glob "**/*.test.ts" -l 2>nul || echo ""');
  ...
  const hasProtectedData = auditReport.findings.tutorial_sections?.total_rows > 0 ||
    Object.values(auditReport.findings.legacy_child_tables || {}).some((t: any) => t.row_count > 0) ||
    Object.values(auditReport.findings.backup_tables || {}).some((t: any) => t.row_count > 0);
  ...
  auditReport.executive_summary = {
    ...
    key_findings: {
      ...
      child_tables_empty: Object.values(auditReport.findings.legacy_child_tables || {}).every((t: any) => t.row_count === 0 || t.verdict === 'TABLE_NOT_FOUND'),
      backup_tables_empty: Object.values(auditReport.findings.backup_tables || {}).every((t: any) => t.row_count === 0 || t.verdict === 'TABLE_NOT_FOUND'),
  ...
  if (auditReport.executive_summary.warnings.length > 0) {
    ...
    auditReport.executive_summary.warnings.forEach((w: string) => console.log(`   - ${w}`));
    console.log('');
  }

  if (auditReport.executive_summary.surprises.length > 0) {
    console.log('🔍 SURPRISES:');
    auditReport.executive_summary.surprises.forEach((s: string) => console.log(`   - ${s}`));
    console.log('');
  }
...
main().catch((error) => {
  console.error('\n❌ Fatal error during Phase A audit:', error);
  process.exit(1);
});
