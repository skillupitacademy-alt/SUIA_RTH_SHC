...
try {
  ...
  if (multibrands.length === 0) {
    ...
    multibrands.forEach((row: any, i: number) => {
  ...
  if (anyMultibrand.length === 0) {
    ...
    anyMultibrand.forEach((row: any, i: number) => {
  ...
  brandDist.forEach((row: any) => {
    console.log(`   • ${String(row.brand_id).padEnd(20)} — ${row.topic_count} topics, ${row.sidebar_count} sidebars`);
  });
  console.log('');
