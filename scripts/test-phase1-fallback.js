...
const REQUEST_TIMEOUT_MS = Number(process.env.PHASE1_FALLBACK_TIMEOUT_MS || 15000);
...
function assertHealthyPage(brand, path, response) {
  if (response.status >= 500) {
    throw new Error(`${brand.name} ${path} returned HTTP ${response.status}`);
  }
  ...
  if (response.status === 401 || response.status === 403) {
    throw new Error(`${brand.name} ${path} rejected authenticated user: HTTP ${response.status}`);
  }
}

async function testBrand(brand) {
  console.log(`\nChecking ${brand.name} fallback paths`);
  const cookie = await login(brand);
  console.log('  Login: PASS');

  for (const path of ['/dashboard', '/dashboard/profile', '/api/auth/me']) {
    const response = await request(brand.host, path, 'GET', null, cookie);
    assertHealthyPage(brand, path, response);
    console.log(`  ${path}: PASS (${response.status})`);
  }
}

async function main() {
  console.log('PHASE 1 FALLBACK VALIDATION');
  console.log('====================================');
  ...
  console.log('\nPHASE 1 FALLBACK VALIDATION PASSED');
}

main().catch((error) => {
  console.error('\nPHASE 1 FALLBACK VALIDATION FAILED');
