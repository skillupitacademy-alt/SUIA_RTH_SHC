...
if (!email) {
  console.error('Usage: node --loader tsx scripts/check-export-data.ts <user-email>');
  process.exit(1);
}
...
async function main() {
  ...
  try {
    const examRes = await client.query<ExamRow>(
      `
