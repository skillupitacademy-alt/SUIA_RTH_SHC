...
export const getPlacementDb = (type: 'primary' | 'direct' = 'primary'): DbClient => {
  const databaseUrl =
    type === 'direct' ? process.env.DATABASE_DIRECT_URL_PLACEMENT : process.env.DATABASE_URL_PLACEMENT;
  ...
  if (databaseUrl === undefined || databaseUrl.trim().length === 0) {
    ...
    throw new Error(
      type === 'direct'
        ? 'DATABASE_DIRECT_URL_PLACEMENT environment variable is required'
        : 'DATABASE_URL_PLACEMENT environment variable is required'
    );
  }
...
export const db = new Proxy({} as DbClient, {
  get: (target, prop) => {
    ...
    return Reflect.get(getPlacementDb('primary') as object, prop);
  },
});
...
export const dbDirect = new Proxy({} as DbClient, {
  get: (target, prop) => {
    ...
    return Reflect.get(getPlacementDb('direct') as object, prop);
  },
});
