...
export const tutorialProjectSubmissions = pgTable('tutorial_project_submissions', {
  ...
  aiReview: jsonb('ai_review').$type<Record<string, unknown> | null>().default(null),
  peerReviews: jsonb('peer_reviews').$type<Record<string, unknown>[]>().notNull().default([]),
  adminReview: jsonb('admin_review').$type<Record<string, unknown> | null>().default(null),
