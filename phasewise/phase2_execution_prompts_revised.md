...
> 53 tasks organized into **7 Sprints** by dependency order.
> Each prompt is ready to paste into an AI coding agent.

> [!IMPORTANT]
> **Sprint 7 (Tasks 82 & 88) is USER-GATED.** These are the only tasks with potential layout shift risk.
> They will NOT be executed until the user explicitly approves them after all other sprints are complete.
...
### ~~Task 82: next/font~~ → **Moved to Sprint 7 (User-Gated)**
...
### ~~Task 88: Tailwind Preset~~ → **Moved to Sprint 7 (User-Gated)**
...
## 🔒 Sprint 7: Visual-Risk Tasks — USER APPROVAL REQUIRED

> [!CAUTION]
> **DO NOT execute these tasks without explicit user approval.**
> These are the only 2 tasks in Phase 2 that could cause layout shift or visual regression.
> Execute them LAST, after all Sprints 1-6 are complete and verified.

---

### Task 82: `next/font` Optimization — `NOT STARTED` | ⚠️ LAYOUT RISK

**Risk**: Font metrics could differ slightly, causing text reflow.
**Mitigation**: `font-display: swap` + same fonts (Inter, Outfit) — should *eliminate* CLS, not create it.
...
> - [ ] Take screenshots of all 3 apps (web, admin, api) before execution
> - [ ] Note current font rendering in browser DevTools
>
> **Post-execution verification:**
> - [ ] Compare screenshots — no text reflow or layout shift
> - [ ] Network tab: no external Google Fonts requests
> - [ ] `pnpm build` exits 0 for all apps

---

### Task 88: Tailwind Preset Deduplication — `NOT STARTED` | ⚠️ STYLE RISK

**Risk**: Shared preset could miss app-specific Tailwind classes, breaking styles.
**Mitigation**: Run apps side-by-side before/after. Diff the Tailwind configs first.

**AI Prompt:**

> 1. Read `apps/web-app/tailwind.config.ts` and `apps/admin-app/tailwind.config.ts`
> 2. Diff them line-by-line to identify identical vs app-specific config
> 3. Create `packages/ui/tailwind.preset.ts` with ONLY the identical parts
> 4. Update both app configs to use `presets: [sharedPreset]`
> 5. Keep ALL app-specific overrides in their own config
> 6. Keep `content` paths pointing to their own files + `packages/ui/src/**/*.tsx`
>
> **Pre-execution checklist (for user):**
> - [ ] Take screenshots of all key pages in both apps before execution
> - [ ] Note any app-specific colors or animations
>
> **Post-execution verification:**
> - [ ] Compare screenshots — zero visual differences
> - [ ] `pnpm build` exits 0 for all apps
> - [ ] Spot-check 5 pages in each app for style correctness
