 ...
 */
export function NotesConceptMemoryMap({ image, nodes, connections }: NotesConceptMemoryMapProps) {
  const brand = useBrand();

  const dataUri = typeof image === 'string' ? image : image?.dataUri;
  const altText = typeof image === 'object' ? image?.alt : 'Concept Memory Map';

  if ((!nodes || nodes.length === 0) && !dataUri) return null;
