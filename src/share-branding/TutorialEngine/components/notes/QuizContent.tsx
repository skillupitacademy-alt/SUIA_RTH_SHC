  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
      ...
      <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-400">
         <span>JavaScript</span> <Icons.ChevronRight size={10} /> <span>React Development</span> <Icons.ChevronRight size={10} /> <span>Component Architecture</span> <Icons.ChevronRight size={10} /> <span className="text-slate-600">2.9 Quiz</span>
      </div>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
          ...
          <div className="flex items-center gap-4">
             ...
             <div>
                <div className="flex items-center gap-2">
                   <span className="rounded-md bg-rose-100 px-2 py-0.5 text-[10px] font-black uppercase text-rose-600">Quiz</span>
                   <h1 className="text-2xl font-black text-slate-800 tracking-tight">2.9 Quiz: Component Architecture - Intermediate</h1>
                </div>
                <p className="text-[14px] font-medium text-slate-500">Test your understanding of component composition, props flow, and modular design patterns.</p>
             </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-8">
           <div className="flex items-center gap-2 text-slate-400">
              ...
              <span className="text-sm font-black text-slate-600">10 Questions</span>
           </div>
           <div className="flex items-center gap-2 text-slate-400">
              <Icons.Clock size={18} />
              <span className="text-sm font-black text-slate-600">15 min</span>
           </div>
           <div className="flex items-center gap-2 text-rose-500">
              <Icons.Trophy size={18} fill="currentColor" />
      ...
      <div className="space-y-4">
         <div className="flex items-center justify-between">
            <p className="text-[12px] font-black text-slate-400 uppercase tracking-widest">Your Progress</p>
            <p className="text-sm font-black text-slate-800">40%</p>
         </div>
      ...
      <section className="rounded-[32px] bg-white p-10 shadow-xl border border-slate-200 space-y-8 relative group transition-all duration-300 hover:-translate-y-1">
         <div className="flex items-center justify-between">
            ...
            <button className="flex items-center gap-2 text-[12px] font-black text-slate-400 hover:text-slate-600 transition-colors">
               <Icons.Bookmark size={16} /> Mark for Review
            </button>
         </div>

         <h2 className="text-xl font-black text-slate-800 leading-tight">What will be the output of the following code?</h2>

         {/* Code Block */}
         <div className="rounded-2xl bg-[#0f172a] p-6 font-mono text-sm leading-relaxed overflow-hidden shadow-2xl relative">
            <div className="absolute top-0 right-0 p-2 opacity-20">
               <Icons.Code size={40} className="text-slate-400" />
            </div>
                  ...
                  <div key={i} className="flex gap-4">
                     ...
                     <span>
                        {line.includes('//') ? <span className="text-emerald-500/60 italic">{line}</span> :
                         line.includes('Promise') || line.includes('Error') ? <span className="text-amber-400">{line}</span> :
                         line.includes('then') || line.includes('catch') ? <span className="text-pink-400">{line}</span> :
                         line}
                     </span>
                  </div>
               ))}
            ...
            {options.map((opt) => (
               <button
                ...
                >
                  ...
                  <span className={`text-sm font-black ${selectedOption === opt.id ? 'text-rose-700' : 'text-slate-600'}`}>
                     <span className="mr-4 text-[12px] opacity-50 font-black">{opt.id}</span>
                     {opt.text}
                  </span>
               </button>
            ))}
         ...
         {selectedOption === 'C' && (
            <div className="rounded-[24px] bg-rose-50/50 p-8 flex items-start gap-6 border border-slate-200 relative overflow-hidden animate-in fade-in slide-in-from-top-4 duration-500 shadow-xl transition-all duration-300 hover:shadow-2xl">
               ...
               <div className="space-y-2 relative z-10">
                  <h4 className="text-sm font-black text-rose-700">Explanation</h4>
                  <p className="text-[13px] font-bold text-rose-900/70 leading-relaxed">
      ...
      <div className="flex items-center justify-between gap-4 pt-10 border-t border-slate-100">
         <div className="flex items-center gap-3 text-[12px] font-bold text-slate-500">
            <Icons.Zap size={16} className="text-amber-500" fill="currentColor" />
            <p>Tip: Review the Promise Chain execution order in the <span className="text-slate-800 font-black">Technical Deep Dive</span> section.</p>
         </div>
         <button className="flex items-center gap-2 text-[12px] font-black text-rose-500 hover:text-rose-600 transition-colors uppercase tracking-widest">
            Go to Section <Icons.ArrowRight size={14} />
         </button>
      </div>
    </div>
  );
