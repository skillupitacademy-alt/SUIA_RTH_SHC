...
export function NotesLeftSidebar({ data, isOpen, activeId, onSelect }: { data: SubtopicNotesViewData['leftSidebar']; isOpen: boolean; activeId: string; onSelect: (id: string) => void }) {
  ...
  return (
    <aside aria-label="Learning Path Sidebar" className={`absolute bottom-0 left-0 top-0 z-40 flex w-[280px] flex-col overflow-y-auto border-r border-gray-200 bg-white p-5 hide-scrollbar transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0`}>
